...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        """
        print("Computing integral images") # For Fast Calculations
        posNum, negNum = 0, 0
        iis, labels = [], []
        ...
        for t in range(self.T):
            ...
            #update weights
            accuracy = []
            for x, y in zip(iis, labels): # zip() - make two lists into a list of tuples
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        for i in range(len(features)):
            error = 0
            hclass = WeakClassifier(feature = features[i])
